...
#include "CROWZ_CoreUObject_structs.hpp"
...
public:
	...
	{
		const auto nameLen = StringUtils::Len(name);
		for (int i = 0; i < GetGlobalObjects().Num(); ++i)
		{
			...
			char fullName[1024];
			fullName[0] = 0;
			const auto len = object->GetFullName(fullName);
			if (len == -1) continue;

			if(findFirstBest ? StringUtils::CompareSub(fullName, name, 0, nameLen)
				: StringUtils::Equals(name, fullName))
			{
